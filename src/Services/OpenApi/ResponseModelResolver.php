...
class ResponseModelResolver
{
    ...
    public function resolveReference(string $class): OpenApi\Reference
    {
        ...
        if (!$typename) {
            if (!RestApiBundle\Helper\InterfaceChecker::isResponseModelInterface($class)) {
    ...
    private function resolveByType(PropertyInfo\Type $type)
    {
        switch (true) {
            ...
            case $type->getBuiltinType() === PropertyInfo\Type::BUILTIN_TYPE_OBJECT && RestApiBundle\Helper\InterfaceChecker::isResponseModelInterface($type->getClassName()):
            ...
            case $type->getBuiltinType() === PropertyInfo\Type::BUILTIN_TYPE_OBJECT && RestApiBundle\Helper\InterfaceChecker::isDateTimeInterface($type->getClassName()):
            ...
            case $type->getBuiltinType() === PropertyInfo\Type::BUILTIN_TYPE_OBJECT && RestApiBundle\Helper\InterfaceChecker::isResponseModelEnumInterface($type->getClassName()):
                $result = $this->resolveEnum($type);

                break;

            case $type->getBuiltinType() === PropertyInfo\Type::BUILTIN_TYPE_OBJECT && RestApiBundle\Helper\InterfaceChecker::isResponseModelDateInterface($type->getClassName()):
