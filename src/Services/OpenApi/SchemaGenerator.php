...
class SchemaGenerator
{
    ...
    private function createOperation(RestApiBundle\Model\OpenApi\EndpointData $endpointData, string $httpMethod, string $routePath): OpenApi\Operation
    {
        ...
        foreach ($endpointData->reflectionMethod->getParameters() as $reflectionMethodParameter) {
            ...
            } elseif ($reflectionMethodType->getClassName() && RestApiBundle\Helper\InterfaceChecker::isMapperModelInterface($reflectionMethodType->getClassName())) {
                if ($requestModelType) {
    ...
    private function createResponses(\ReflectionMethod $reflectionMethod): OpenApi\Responses
    {
        ...
        if ($returnType->isCollection()) {
            ...
            if (!$collectionValueType->getClassName() || !RestApiBundle\Helper\InterfaceChecker::isResponseModelInterface($collectionValueType->getClassName())) {
            ...
            if (RestApiBundle\Helper\InterfaceChecker::isResponseModelInterface($returnType->getClassName())) {
