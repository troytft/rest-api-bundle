...
class SchemaResolver implements RestApiBundle\Services\Mapper\SchemaResolverInterface
{
    ...
    private function resolveSchemaByType(PropertyInfo\Type $type, array $typeOptions = []): RestApiBundle\Model\Mapper\Schema
    {
        switch (true) {
            ...
            case $type->getClassName() && RestApiBundle\Helper\InterfaceChecker::isMapperDateInterface($type->getClassName()):
                $dateFormat = null;
            ...
            case $type->getClassName() && RestApiBundle\Helper\InterfaceChecker::isDateTimeInterface($type->getClassName()):
                $dateFormat = null;
                foreach ($typeOptions as $typeOption) {
                    if ($typeOption instanceof RestApiBundle\Mapping\Mapper\DateFormat) {
                        $dateFormat = $typeOption->getFormat();
            ...
            case $type->getClassName() && RestApiBundle\Helper\InterfaceChecker::isMapperModelInterface($type->getClassName()):
                $schema = $this->resolve($type->getClassName(), $type->isNullable());

                break;

            case $type->getClassName() && RestApiBundle\Helper\InterfaceChecker::isMapperEnumInterface($type->getClassName()):
